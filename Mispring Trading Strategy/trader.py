...
def plot_summary(df, title):
    f, (a0, a1, a2, a3, a4) = plt.subplots(5, 1, gridspec_kw={'height_ratios': [2, 1, 1, 1,1]})
    ...
    a0.set_title("realized pnl vs. unrealized pnl")
    a0.plot(df["unrealized_pnl_cum"], label="cum unrealized pnl")
    a0.plot(df["realized_pnl_cum"], label="cum realized pnl")
    a0.legend()

    a1.set_title("cum rezlized return %")
    a1.plot(df["ret_cum"])

    a2.set_title("holdings")
    a2.plot(df["position_unit"])

    a3.set_title("realized max drawdown")
    a3.plot(df["realized_max_drawdown"], label="max drawdown")
    a3.plot(df["realized_avg_drawdown"], label="avg drawdown")

    a4.set_title("unrealized max drawdown")
    a4.plot(df["unrealized_max_drawdown"], label="max drawdown")
    a4.plot(df["unrealized_avg_drawdown"], label="avg drawdown")
...
def plot_max_drawdown(df):
    ...
    a0.set_title("realized max drawdown")
    a0.plot(df["realized_max_drawdown"], label="max drawdown")
    a0.plot(df["realized_avg_drawdown"], label="avg drawdown")
    a0.legend()

    a1.set_title("unrealized max drawdown")
    a1.plot(df["unrealized_max_drawdown"], label="max drawdown")
    a1.plot(df["unrealized_avg_drawdown"], label="avg drawdown")
    a1.legend()
...
def get_summary_copula(df, prices_all):
    years = prices_all.index.year.unique()
    return get_summary_mispricing(df, [prices_all.columns[0],prices_all.columns[1]],years[0], years[-2], years[-1])

def get_summary_mispricing(df, pair, training_start, training_end, testing_period):
    df_summary = [("x y pair", pair[0]+" "+pair[1]),
                    ("training period", str(training_start)+"-"+str(training_end)),
                    ("testing period", testing_period),
                    ("pct return", df["ret_cum"][-1]-1),
                    ("return std", df["ret_daily"].std()),
                    ("realized pnl", df["realized_pnl_cum"][-1]),
                    ("num of transactions", df["action_unit"].abs().sum()),
                    ("max long holding", df["position_unit"].max()),
                    ("max short holding", df["position_unit"].min())]
    df = pd.DataFrame(df_summary,columns=["item", "value"])
    return df
...
# print(get_summary_copula(return_df1, prices_all))
# # print(os.getcwd())