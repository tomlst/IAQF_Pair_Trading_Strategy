...
def plot_summary(df, title):
    ...
    a0.set_title("P&L, Realized P&L, Unrealized P&L")
...
def get_summary_mispricing(df, pair, training_start, training_end, testing_period):
    df_summary = {"x y pair": pair[0]+" "+pair[1],
                    ...
                    "return std": df["unrealized_pnl_cum"].fillna(0).std(),
                    # "log return std": np.log(df["unrealized_pnl_cum"].pct_change()+1).fillna(0).std(),
                    "period end realized pnl": df["realized_pnl_cum"][-1],
                    "num of transactions": df["action_unit"].abs().sum(),
                    "max long holding": df["position_unit"].max(),
                    "max short holding": df["position_unit"].min()}
    # df = pd.from_dict(df_summary,columns=["item", "value"])
    return df_summary
